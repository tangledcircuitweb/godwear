...
describe('R2 Bucket Lifecycle Management', () => {
  ...
  test('should create and use R2 bucket successfully', async () => {
    ...
    const downloadedContent = execSync(`wrangler r2 object get ${testBucketName}/${testKey} --pipe`, {
      encoding: 'utf8',
      stdio: 'pipe',
      timeout: 30000
    });
  ...
  test('should handle multiple files in bucket', async () => {
    ...
    for (const file of files) {
      execSync(`echo "${file.content}" | wrangler r2 object put ${testBucketName}/${file.key} --pipe`, {
        stdio: 'pipe',
        timeout: 30000
      });
    }
    ...
    for (const file of files) {
      const downloadedContent = execSync(`wrangler r2 object get ${testBucketName}/${file.key} --pipe`, {
        encoding: 'utf8',
        stdio: 'pipe',
        timeout: 30000
      });
  ...
  test('should handle bucket operations without conflicts', async () => {
    ...
    // Upload content
    execSync(`echo "${uniqueContent}" | wrangler r2 object put ${testBucketName}/${testKey} --pipe`, {
      stdio: 'pipe',
      timeout: 30000
    });
...
describe('Shared R2 Bucket Tests', () => {
  ...
  test('should upload file to shared bucket', async () => {
    ...
    execSync(`echo "${testContent}" | wrangler r2 object put ${sharedBucketName}/${testKey} --pipe`, {
      stdio: 'pipe',
      timeout: 30000
    });
    
    const downloadedContent = execSync(`wrangler r2 object get ${sharedBucketName}/${testKey} --pipe`, {
      encoding: 'utf8',
      stdio: 'pipe',
      timeout: 30000
    });
  ...
  test('should see file from previous test', async () => {
    ...
    const downloadedContent = execSync(`wrangler r2 object get ${sharedBucketName}/shared-file-1.txt --pipe`, {
      encoding: 'utf8',
      stdio: 'pipe',
      timeout: 30000
    });
