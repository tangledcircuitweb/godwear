...
import { createTestFactory, fixtures, aUser, aRequest, aResponse } from './helpers/test-factory';
  ...
  describe('Data Builders', () => {
    ...
    it('should build responses with different statuses', () => {
      const successResponse = aResponse()
        .asSuccess({ message: 'OK' })
        .build();
      
      expect(successResponse.status).toBe(200);
      
      const errorResponse = aResponse()
        .asError('TEST_ERROR', 'Test error message', 400)
        .build();
