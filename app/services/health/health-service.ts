...
export class HealthService implements BaseService {
  ...
  async getSystemHealth(): Promise<SystemHealthStatus> {
    ...
    const dependencies = {
      ...
      google_oauth: (this.env.GOOGLE_CLIENT_ID && this.env.GOOGLE_CLIENT_SECRET) ? 'healthy' as const : 'unhealthy' as const,
    };

    // Determine overall status
    const unhealthyServices = Object.values(dependencies).filter(status => status === 'unhealthy');
    const overall = unhealthyServices.length === 0 ? 'healthy' as const : 
