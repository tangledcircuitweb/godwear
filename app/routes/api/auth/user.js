...
app.get("/", async (c) => {
    try {
        ...
        if (!c.env.JWT_SECRET) {
            const errorResponse = createErrorResponse(ErrorCodes.SERVICE_CONFIGURATION_ERROR, "Authentication service not configured", undefined, "auth-user");
        ...
        let errorCode = ErrorCodes.AUTH_INVALID_TOKEN;
...
app.post("/refresh", (c) => {
    const errorResponse = createErrorResponse(ErrorCodes.SERVICE_UNAVAILABLE, "Token refresh not implemented yet - please log in again", undefined, "auth-user");
