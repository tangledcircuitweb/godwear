...
app.get("/", zValidator("query", OAuthCallbackSchema.or(OAuthErrorSchema)), async (c) => {
    ...
    try {
        ...
        if ("error" in query) {
            ...
            const errorResponse = createErrorResponse(ErrorCodes.AUTH_OAUTH_ERROR, `OAuth error: ${query.error}`, {
                error: query.error,
        ...
        if (!storedState || storedState !== query.state) {
            ...
            const errorResponse = createErrorResponse(ErrorCodes.AUTH_INVALID_STATE, "Invalid OAuth state parameter - possible CSRF attack", {
        ...
        const errorResponse = createErrorResponse(errorCode, errorMessage, errorDetails, "auth-callback");
